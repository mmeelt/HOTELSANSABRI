...
Evenement::Evenement(const string& date, const string& lieu, const string& description, vector<unique_ptr<TravailleurSocialEvenementiel>> resp)
...
vector<unique_ptr<TravailleurSocialEvenementiel>> Evenement::getResp() const {
    return move(resp);
}

void Evenement::setResp(vector<unique_ptr<TravailleurSocialEvenementiel>> resp) {
    this->resp = move(resp);
}
